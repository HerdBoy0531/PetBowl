...
export async function GET(req: Request) {
  ...
  try {
    ...
    // FoodViewLog에서 조건에 맞는 데이터를 그룹화하여 카운팅 연산
    ...
    // 집계된 foodId 배열을 추출하여 실제 사료들의 세부 마스터 정보 매핑 조회
    ...
    // groupBy의 순서(순위)가 findMany 조회 시 섞이므로 원래 순위 배열대로 정렬 재조립
    const rankedResult = groupRows
      .map((row) => {
        ...
        return {
          id: row.foodId,
          // 단일 name/brand 대신 Ko, En을 명확히 구분하여 전달 (메인 카드 정보 정립)
          ...
          // 메인 페이지 카드 컴포넌트 정보 정립을 위한 필수 메타데이터 수수료 추가
