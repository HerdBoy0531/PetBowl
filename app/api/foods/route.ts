...
export async function GET(req: Request) {
  ...
  // 이름순, 가격순 정렬 조건 처리 (인기순은 프론트에서 랭킹 API 연동 또는 추후 로그 연동)
  ...
  const kibbleSizeParam = searchParams.get("kibbleSize");
  ...
  // 키블 크기 다중 필터 (유저가 선택한 mm 크기 배열 일치 검색)
  if (kibbleSizeParam) {
  ...
  // 알레르기 제어 Enum 다중 배열 검색 (AND 조건 처리: 선택한 알러지 케어를 전부 만족하는 제품)
  if (allergiesParam) {
  ...
  // 인증마크 Enum 다중 배열 검색 (AND 조건 처리: 선택한 인증을 전부 획득한 제품)
  ...
  try {
    ...
    // 정립된 Search Page 결과 카드 규격에 맞춰 결과 가공 전송
